...
export default function SubcontrataProyectoDetailPage() {
  ...
  const getStatus = (p: Proyecto) => {
    const now = new Date();
    if (p.fechaFin && new Date(p.fechaFin) < now) return { text: "Finalizado", color: "bg-gray-500" };
    if (p.fechaInicio && new Date(p.fechaInicio) > now) return { text: "Próximamente", color: "bg-blue-500" };
