...
import { getProyectoById, getSubcontratas, updateProyecto } from '@/lib/actions/app.actions';
...
export default function EditProyectoPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const [proyectoData, subcontratasData] = await Promise.all([
        ...
        form.reset({
            ...
            fechaFin: proyectoData.fechaFin ? new Date(proyectoData.fechaFin) : undefined,
        });
  ...
  const handleSubmit = async (data: EditProyectoFormData) => {
    ...
    const result = await updateProyecto(projectId, data);
    if(result.success) {
        toast({ title: "Éxito", description: "Proyecto actualizado correctamente."});
        router.push(`/constructora/proyectos/${projectId}`);
    } else {
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <Controller name="fechaFin" control={form.control} render={({ field }) => (
                                ...
                                <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal mt-1", !field.value && "text-muted-foreground")}>
                                    <CalendarIcon className="mr-2 h-4 w-4" />{field.value ? format(field.value, "PPP", {locale: es}) : <span>Selecciona fecha</span>}
                                </Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={field.value ?? undefined} onSelect={field.onChange} initialFocus locale={es}/></PopoverContent></Popover>
                            )}/>
                        </div>
                    </div>
