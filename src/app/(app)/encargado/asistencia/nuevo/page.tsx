...
import { getProyectosBySubcontrata, getTrabajadoresByProyecto, getSubcontratas, saveDailyReport } from '@/lib/actions/app.actions';
import type { Proyecto, Trabajador, Subcontrata, ReporteTrabajador } from '@/lib/types';
...
export default function NuevaAsistenciaPage() {
  ...
  const handleSaveAttendance = async () => {
    ...
    const attendedWorkers: ReporteTrabajador[] = trabajadores
      .filter(t => t.asistencia)
      .map(t => ({
        trabajadorId: t.id,
        nombre: t.nombre,
        asistencia: true,
        horas: 8, // Default 8 hours for simple attendance
      }));

    if (attendedWorkers.length === 0) {
      toast({
        title: "Atención",
        description: "No se puede guardar un registro sin trabajadores presentes.",
        variant: "destructive"
      });
    ...
    const result = await saveDailyReport(selectedProyecto, currentUserId, attendedWorkers, "Registro de asistencia rápido desde el módulo de Asistencia.");
    
    if(result.success) {
        toast({ title: "Éxito", description: "Asistencia guardada. Redirigiendo al historial..." });
  ...
  return (
    <div className="space-y-8">
       <div>
        <h1 className="text-3xl font-bold font-headline text-primary">Nuevo Registro de Asistencia</h1>
