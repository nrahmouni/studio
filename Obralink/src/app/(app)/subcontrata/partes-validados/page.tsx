...
export default function SubcontrataPartesValidadosPage() {
  ...
  const generatePDF = (reporte: ReporteDiario) => {
    ...
    doc.text(`Fecha: ${format(reporte.fecha, "PPPP", { locale: es })}` , 14, 36);
    ...
    if(reporte.validacion.encargado.timestamp) {
      doc.text(`- Encargado: Validado el ${format(reporte.validacion.encargado.timestamp, "Pp", {locale: es})}`, 16, finalY);
      finalY += 6;
    }
    if(reporte.validacion.subcontrata.timestamp) {
       doc.text(`- Subcontrata: Validado el ${format(reporte.validacion.subcontrata.timestamp, "Pp", {locale: es})}`, 16, finalY);
       finalY += 6;
    }
    if (reporte.validacion.constructora.validado && reporte.validacion.constructora.timestamp) {
        doc.text(`- Constructora: Validado el ${format(reporte.validacion.constructora.timestamp, "Pp", {locale: es})}`, 16, finalY);
    } else {
    ...
    // Save the PDF
    doc.save(`Reporte-${proyectoNombre.replace(/ /g, '_')}-${format(reporte.fecha, 'yyyy-MM-dd')}.pdf`);
  };
