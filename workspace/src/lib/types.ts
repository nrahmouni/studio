...
export const EmpresaSchema = z.object({
  ...
  CIF: z.string().min(9, "El CIF debe tener al menos 9 caracteres.").regex(/^[A-HJ-NP-SUVW]{1}[0-9]{7}[0-9A-J]{1}$/i, "Formato de CIF inválido.").optional().nullable(),
...
export const UsuarioFirebaseSchema = z.object({
  id: z.string(),
  nombre: z.string().min(1, "El nombre es requerido").optional().nullable(),
  email: z.string().email("Email inválido"),
  dni: z.string().min(9, "El DNI debe tener 9 caracteres").regex(/^[0-9XYZxyz][0-9]{7}[A-HJ-NP-TV-Z]$/i, "Formato de DNI/NIE inválido (e.g., 12345678A o X1234567B)").optional().nullable(),
