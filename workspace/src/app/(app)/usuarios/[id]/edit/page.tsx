...
// Schema for editing, DNI and Nombre can be null initially
const UsuarioEditFormSchema = UsuarioFirebaseSchema.omit({ 
  id: true, 
  empresaId: true, 
  password: true 
}).extend({
  nombre: UsuarioFirebaseSchema.shape.nombre, // Keep validation from base schema, but it's optional nullable
  dni: UsuarioFirebaseSchema.shape.dni, // Keep validation from base schema, but it's optional nullable
...
export default function EditUsuarioPage() {
  ...
  const form = useForm<UsuarioEditFormData>({
    ...
    defaultValues: {
      nombre: '', // Will be initialized with null or value
      email: '',
      dni: '', // Will be initialized with null or value
      rol: 'trabajador', 
      activo: true,
      obrasAsignadas: [],
    },
    ...
    const fetchUsuario = async () => {
      ...
      try {
        ...
        if (fetchedUsuario) {
          ...
          form.reset({
            nombre: fetchedUsuario.nombre || '', // Handle null from DB
            email: fetchedUsuario.email,
            dni: fetchedUsuario.dni || '', // Handle null from DB
  ...
  const onSubmit = async (data: UsuarioEditFormData) => {
    ...
    try {
      ...
      // Ensure empty strings for optional fields become null if that's the DB expectation
      const dataToSubmit = {
          ...updateData,
          nombre: updateData.nombre === '' ? null : updateData.nombre,
          dni: updateData.dni === '' ? null : updateData.dni,
      };
      const result = await updateUsuario(usuarioId, currentEmpresaId, dataToSubmit);
      if (result.success && result.usuario) {
        toast({ title: 'Éxito', description: `Usuario "${result.usuario.nombre || 'Sin Nombre'}" actualizado.` });
      ...
      <Card className="max-w-lg mx-auto shadow-lg animate-fade-in-up">
        ...
        <CardContent className="p-6">
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div>
              <Label htmlFor="nombre" className="font-semibold">Nombre Completo (opcional en registro inicial)</Label>
              <Controller
                name="nombre"
                control={form.control}
                render={({ field }) => <Input id="nombre" {...field} value={field.value ?? ''} placeholder="Ej: Ana García Pérez" className="mt-1" />}
              />
            ...
            <div>
              <Label htmlFor="dni" className="font-semibold">DNI / NIE (opcional en registro inicial)</Label>
              <Controller
                name="dni"
                control={form.control}
                render={({ field }) => <Input id="dni" {...field} value={field.value ?? ''} placeholder="Ej: 12345678A o X1234567B" className="mt-1" />}
              />
            ...
            {obrasAsignadasCurrent && obrasAsignadasCurrent.length > 0 && (
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        {obrasAsignadasCurrent.join(', ')} (Se gestionan desde la edición de cada obra)
                    </p>
                </div>
            )}
