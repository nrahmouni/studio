...
const RegisterTrabajadorFormSchema = UsuarioFirebaseSchema.pick({
  ...
  // DNI itself can be null/optional now from the base schema
  nombre: z.string().min(1, "El nombre es requerido"), // Keep it required for new worker registration
  dni: UsuarioFirebaseSchema.shape.dni.unwrap().unwrap(), // Keep DNI required for new worker registration
...
export default function CompanyProfilePage() {
  ...
  const empresaForm = useForm<UpdateEmpresaFormData>({
    ...
    defaultValues: {
      nombre: '',
      CIF: '', // Will be initialized with null or value
      emailContacto: '', // Will be initialized with null or value
      telefono: '', // Will be initialized with null or value
      logoURL: '',
    },
  });
  ...
  const trabajadorForm = useForm<RegisterTrabajadorFormData>({
    ...
    defaultValues: {
      nombre: '',
      email: '',
      dni: '', // DNI for new worker is required
      dniAnversoURL: '',
      dniReversoURL: '',
    },
  });
  ...
  const onEmpresaSubmit = async (data: UpdateEmpresaFormData) => {
    ...
    try {
      // Ensure empty strings are converted to null for optional fields if necessary
      const dataToSubmit = {
        ...data,
        CIF: data.CIF === '' ? null : data.CIF,
        emailContacto: data.emailContacto === '' ? null : data.emailContacto,
      ...
      if (result.success && result.empresa) {
        ...
        empresaForm.reset({ // Reset with potentially null values from DB
            nombre: result.empresa.nombre || '',
            CIF: result.empresa.CIF || '',
            emailContacto: result.empresa.emailContacto || '',
  ...
  const onTrabajadorSubmit = async (data: RegisterTrabajadorFormData) => {
    ...
    try {
      const dataToSend = {
        ...data,
        // DNI is required in this form, so it will be a string
          ...
          <form onSubmit={empresaForm.handleSubmit(onEmpresaSubmit)} className="space-y-6">
            <div>
              ...
              <Controller
                ...
                render={({ field }) => <Input id="nombre" {...field} value={field.value ?? ''} className="mt-1" placeholder="Ej: Construcciones Modernas S.L." />}
              />
            ...
            <div>
              <Label htmlFor="CIF" className="font-semibold">CIF (opcional en registro inicial)</Label>
              <Controller
                name="CIF"
                control={empresaForm.control}
                render={({ field }) => <Input id="CIF" {...field} value={field.value ?? ''} className="mt-1" placeholder="Ej: B12345678" />}
              />
              {empresaForm.formState.errors.CIF && <p className="text-sm text-destructive mt-1">{empresaForm.formState.errors.CIF.message}</p>}
            </div>

            <div>
              <Label htmlFor="emailContacto" className="font-semibold">Email de Contacto (opcional en registro inicial)</Label>
              <Controller
                name="emailContacto"
                control={empresaForm.control}
                render={({ field }) => <Input id="emailContacto" type="email" {...field} value={field.value ?? ''} className="mt-1" placeholder="Ej: contacto@empresa.com" />}
              />
            ...
            <div>
              <Label htmlFor="telefono" className="font-semibold">Teléfono (opcional en registro inicial)</Label>
              <Controller
                name="telefono"
                control={empresaForm.control}
                render={({ field }) => <Input id="telefono" {...field} value={field.value ?? ''} className="mt-1" placeholder="Ej: 912345678" />}
              />
          ...
          <form onSubmit={trabajadorForm.handleSubmit(onTrabajadorSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <Controller
                name="dni"
                control={trabajadorForm.control}
                render={({ field }) => <Input id="trabajadorDni" {...field} value={field.value ?? ''} className="mt-1" placeholder="Ej: 12345678A o X1234567B" />}
              />
